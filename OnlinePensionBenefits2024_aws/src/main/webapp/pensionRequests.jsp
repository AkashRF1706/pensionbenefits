...
<script>
	function promptForRejectionReason(employeeId, emailId, employeeName, action, rejectValue) {
		...
		if (reason != null && reason != "") {
			...
			var inputEmployeeName = document.createElement("input");
			inputEmployeeName.type = "hidden";
			inputEmployeeName.name = "employeeName";
			inputEmployeeName.value = employeeName;
			form.appendChild(inputEmployeeName);
...
<body>
	...
	<div class="main">
	<%if(role.equalsIgnoreCase("Company")){ %>
	<a href="companyTransfers.jsp" class="transfer-button">Manage Transfers</a>
	<%} %>
		<%
		...
		<div id="PendingRequests" class="tab-content active">
			...
			try {
				...
				if (role.equalsIgnoreCase("Administrator")) {
					sql = "SELECT e.id, e.first_name, e.last_name, e.email, p.withdraw_pension_amount, p.pension_amount, e.company, po.withdraw_reason FROM employees e "
					+ "JOIN pensions p ON e.id = p.employee_id JOIN pension_operations po on po.employee_id = e.id WHERE p.withdraw_flag = 'A' AND po.status = 'Admin Pending' and po.operation_type = 'Withdraw'";
				} else {
					sql = "SELECT e.id, e.first_name, e.last_name, e.email, p.withdraw_pension_amount, p.pension_amount, po.withdraw_reason FROM employees e "
					+ "JOIN pensions p ON e.id = p.employee_id JOIN pension_operations po on po.employee_id = e.id WHERE p.withdraw_flag = 'C' AND e.company = ? AND po.status = 'Company Pending' and po.operation_type = 'Withdraw'";
				}
			...
			<table>
				...
				<tbody>
					...
					while (rs.next()) {
						...
						String withdrawReason = rs.getString("withdraw_reason");
					%>
					<tr>
						<td><%=employeeId%></td>
						<td><%=employeeName%></td>
						...
						<td>
							...
							<form style="display: inline-block;">
								<button type="button" class="reject"
									onclick="promptForRejectionReason('<%=employeeId%>', '<%=email %>', '<%=employeeName %>', 'ApproveRejectWithdrawServlet', '<%=rejectValue%>')">Reject</button>
							</form>
						</td>
					</tr>
					<%
				...
				<tbody>
					...
					<tr>
						...
						<td>
							...
							<form style="display: inline-block;">
								<button type="button" class="reject"
									onclick="promptForRejectionReason('<%=employeeId%>', '<%=email %>', '<%=employeeName %>', 'ApproveRejectUpdateServlet', 'reject')">Reject</button>
							</form>
						</td>
					</tr>
					<%
